...
class ATSSHead(AnchorHead):
    ...
    def _init_layers(self) -> None:
        ...
        # self.cls_addSE = nn.Sequential(
        #         nn.AdaptiveAvgPool2d((1, 1)),
        #         nn.Flatten(),
        #         nn.Linear(256, 16),  # hidden_dim是全连接层的隐藏层维度
        #         nn.ReLU(),
        #         nn.Linear(16, 256),  # output_dim是全连接层的输出维度
        #         nn.Sigmoid()
        #     )
        # self.reg_addSE = nn.Sequential(
        #         nn.AdaptiveAvgPool2d((1, 1)),
        #         nn.Flatten(),
        #         nn.Linear(256, 16),  # hidden_dim是全连接层的隐藏层维度
        #         nn.ReLU(),
        ...
        self.cj_convs = nn.ModuleList()
        self.cj_convs.append(
                ConvModule(
                    512,
                    self.feat_channels,
                    3,
    ...
    def forward_single(self, x: Tensor, scale: Scale) -> Sequence[Tensor]:
        ...
        for i in range(self.stacked_convs):
            if i != 2:
                ...
                # cls_feat = cls_feat + cls_feat * self.cls_addSE(cls_feat).view(cls_feat.size(0), 256, 1, 1)
                # reg_feat = reg_feat + reg_feat * self.reg_addSE(reg_feat).view(reg_feat.size(0), 256, 1, 1)
                common_feat = torch.cat([cls_feat, reg_feat], dim=1)
                cls_feat = self.cj_convs[0](common_feat)
                reg_feat = self.cj_convs[0](common_feat)
    ...
    def loss_by_feat_single(self, anchors: Tensor, cls_score: Tensor,
        ...
        loss_cls = self.loss_cls(
            cls_score[neg_inds], labels[neg_inds], avg_factor=avg_factor)
        ...
        if len(pos_inds) > 0:
            ...
            pos_cls_s_sigmoid = cls_score[pos_inds, pos_labels].clone().sigmoid()
            ious = bbox_overlaps(
                pos_decode_bbox_pred, pos_bbox_targets, is_aligned=True)
            
            pred_sigmoid = cls_score[pos_inds].sigmoid()
            target = torch.zeros_like(pred_sigmoid)
            target[torch.arange(target.size(0)), pos_labels] = 1

            focal_weight = ((ious.detach() ** 4).unsqueeze(1) * (1 - pred_sigmoid).pow(1) * (target > 0.0).float() +
                            (1 - ious.detach() ** 4).unsqueeze(1) * (pred_sigmoid).pow(1) * (target <= 0.0).float())
            tt = F.binary_cross_entropy(
                pred_sigmoid, target, reduction='none') * focal_weight
            loss_cls = tt.sum() / avg_factor + loss_cls
